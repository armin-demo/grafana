...
func (hs *HTTPServer) UpdateUserEmail(c *contextmodel.ReqContext) response.Response {
	...
	if err := hs.userVerifier.Complete(c.Req.Context(), user.CompleteEmailVerifyCommand{Code: code, User: c.SignedInUser}); err != nil {
...
func (hs *HTTPServer) ChangeUserPassword(c *contextmodel.ReqContext) response.Response {
	...
	if err := hs.AuthTokenService.RevokeAllUserTokens(c.Req.Context(), userID); err != nil {
		hs.log.Error("Failed to revoke user tokens after password change", "userId", userID, "error", err)
	}
